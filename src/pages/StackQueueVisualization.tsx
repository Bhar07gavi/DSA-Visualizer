...
import { ArrowLeft, Plus, Minus, RotateCcw, ArrowUp, ArrowDown, ArrowRight } from "lucide-react";
...
class Stack {
  ...
  toArray(): StackNode[] {
    return [...this.items].reverse(); // Display with top element first
  }
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-muted-foreground">LIFO and FIFO data structure visualization</p>
          </div>
        </div>
                        ...
                        <div className="flex flex-col gap-2 items-center">
                          ...
                          {stackItems.map((item, index) => (
                            <div 
                              key={item.id}
                              className={`
                                w-32 h-16 rounded-lg border-2 transition-all duration-300 flex items-center justify-center font-bold text-lg
                          ...
                          <div className="text-sm text-muted-foreground font-mono">BOTTOM</div>
                        </div>
                      ) : (
                        <div className="text-center">
                          <p className="text-muted-foreground mb-4">Stack is empty</p>
                      ...
                      {queueItems.length > 0 ? (
                        <div className="flex items-center gap-4 min-w-max">
                          ...
                          <div className="flex flex-col items-center gap-2">
                            <span className="text-sm text-muted-foreground font-mono">FRONT</span>
                          ...
                          {queueItems.map((item, index) => (
                            <div key={item.id} className="flex items-center gap-2">
                              <div 
                                className={`
                                  w-20 h-20 rounded-lg border-2 transition-all duration-300 flex items-center justify-center font-bold text-lg
                                  ${highlightedItem === item.id 
                                    ? 'bg-primary border-primary-glow shadow-neon scale-105 text-primary-foreground' 
                                    : 'bg-card border-border/40 text-foreground hover:border-primary/40'
                                  }
                          ...
                          <div className="flex flex-col items-center gap-2 ml-4">
                            <span className="text-sm text-muted-foreground font-mono">REAR</span>
                            <ArrowDown className="w-4 h-4 text-muted-foreground" />
                          </div>
                        </div>
                      ) : (
              ...
              <div className="space-y-6">
                ...
                <Card className="bg-gradient-card border-border/40">
                  ...
                  <CardContent className="space-y-3 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Enqueue time:</span>
                      <span>O(1)</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Dequeue time:</span>
                      <span>O(1)</span>
                    </div>
                  </CardContent>
                </Card>
